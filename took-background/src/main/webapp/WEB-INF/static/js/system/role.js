var roleModule={
	...
	modify : function() {
		...
		$('#save').off('click').on('click',function(){
			$('#form').form('submit', {    
    		    url:ctx+'/system/role/modifyRole', 
    		    onSubmit: function(param){    
    		    	return  $(this).form('validate');  		    	
    		    },    
    		    success:function(dataStr){
	...
	del : function() {
		...
		$.messager.confirm("提示", "您确定删除此菜单吗，删除时，会将所有关联删除，且不可恢复？",
				function(r) {
			 		if(r){
			 			$.ajax({  
				            type : "POST",  
				            url : ctx+"/system/role/delRole/"+row.id,
				            dataType: "json",
				            success : function(result) {
				                if (result.success) {  
				                	messager.show("删除角色成功");
				                	//重新加载列表
			    		    		$('#dg').datagrid('reload');			                	
				                } else {  
