(function($, window) {
	...
	function initWebUpload(item, options) {
		...
		if (opts.auto) {
			...
			uploaderStrdiv = '<div  class="uploader-list"></div>'
					+ '<div class="btns">' + '<div id="' + pickerid
					+ '">选择文件</div>'
					+ '<button class="webuploadbtn">开始上传</button>' + '</div>'
		}
		...
		if (opts.auto) {
			uploader.on('fileQueued', function(file) {
				$list.append('<div id="' + $(item)[0].id + file.id
						+ '" class="item">' + '<span class="webuploadinfo">'
						+ file.name + '</span>'
	...
	$.fn.powerWebUpload = function(options) {
		...
		if (typeof WebUploader == 'undefined') {
			var casspath = ctx+ "/static/plugins/webuploader/webuploader.css";
			$("<link>").attr({
				rel : "stylesheet",
				type : "text/css",
				href : casspath
			}).appendTo("head");
			var jspath = ctx+ "/static/plugins/webuploader/webuploader.min.js";
