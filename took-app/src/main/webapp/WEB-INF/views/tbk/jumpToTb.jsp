...
<body>
	<div style="visibility: hidden;">
		<a data-type="0" biz-itemid="" data-style="2" data-tmpl="230x312"
			style="visibility: hidden;"></a>
	</div>
	<script type="text/javascript">
		(function(win, doc) {
			var s = doc.createElement("script"), h = doc
	...
	<script language="javascript">
		...
		var timer;
		function get_et() {
			var s = new Date(), l = +s / 1000 | 0, r = s.getTimezoneOffset() * 60, p = l
					+ r, m = p + (3600 * 8), q = m.toString().substr(2, 8)
					.split(""), o = [ 6, 3, 7, 1, 5, 2, 0, 4 ], n = [];
		...
		function setCookie(j, k) {
			document.cookie = j + "=" + encodeURIComponent(k.toString())
					+ "; path=/";
		}
		...
		function get_pgid() {
			...
			if (k === "") {
				cu = (s.search.length > 9) ? s.search
						: ((s.pathname.length > 9) ? s.pathname : s.href)
								.substr(1);
				n = document.cookie.split(";");
				for (o = 0; o < n.length; o++) {
		...
		var tl = '230x45';
		var rd = '1';
		var ct = encodeURIComponent('itemid=${itemId}');
		var st = '2';
// 		var rf = encodeURIComponent(document.URL);
		var rf = encodeURIComponent("http://www.tooklili.com");
		...
		function ftxia_jump() {
			...
			try {
				...
				if (fed && fed != 'undefined') {
					var fed = document.getElementById(fd).contentDocument
							|| document.frames[fd].document;
